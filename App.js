...
import {
  ...
  Image, ActivityIndicator, Linking, SafeAreaView, StatusBar,
  TextInput, KeyboardAvoidingView, Platform, Alert, Share, Modal, Dimensions
} from "react-native";
...
import { Menu, Check, X, AlertTriangle, Sparkles, HelpCircle, Camera, Image as ImageIcon, FileText, Mail, LogOut, User, Clock, ShoppingBag, Folder, Share2, Zap, Star, Diamond, Sofa, Shirt, CarFront, UtensilsCrossed, BedDouble, Monitor, Lightbulb, Wrench, Home, ChevronRight } from "lucide-react-native";
import { initializeApp, getApps, getApp } from "firebase/app";
import { initializeAuth, getReactNativePersistence, getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut, onAuthStateChanged, updateProfile } from "firebase/auth";
import { getFirestore, collection, addDoc, doc, updateDoc, getDocs, query, orderBy, limit } from "firebase/firestore";
...
const firebaseConfig = {
  ...
  authDomain: "cluttrd-3e335.firebaseapp.com",
...
const API_KEY = "REDACTED-KEY-ROTATED-AND-PURGED";
const OPENAI_KEY = "REDACTED-KEY-ROTATED-AND-PURGED";
...
function MainApp({ user, isPro, setIsPro, analyses, setAnalyses, setSkipPref }) {
  ...
  const VIZ_TIPS = [
    "AI is analyzing your space layout...",
    "Reimagining your storage solutions...",
    "Placing virtual organizers in your space...",
    "Adjusting lighting and perspective...",
    "Adding finishing touches...",
    "Almost there -- your transformation is nearly ready!",
    "Great spaces take a moment to create...",
    "Visualizing your organized space...",
    "Pro tip: Clear floors make spaces feel twice as large.",
    "Pro tip: Matching containers create instant visual calm.",
    "Pro tip: Labels are the secret to maintaining organization.",
    "Pro tip: Vertical space is the most underused storage area.",
  ];
  ...
  const LOAD_MESSAGES = [
    ...
    "Building your Mid-Range plan…",
    "Building your Premium plan…",
    ...
    try {
      ...
      if (data.error) {
        if (data.error.type === "authentication_error") {
          setErr("There was an authentication issue. Please restart the app and try again.");
        } else if (data.error.type === "rate_limit_error") {
          setErr("We're getting a lot of requests right now. Please wait a moment and try again.");
        } else if (data.error.type === "overloaded_error") {
          setErr("Our AI is a little busy right now. Please try again in a few seconds.");
        } else {
      ...
      if (!match) { setErr("We had trouble reading your space. Try a clearer, well-lit photo."); return; }
  ...
  const generateVisualization = async (tier) => {
    ...
    try {
      ...
      formData.append("image", { uri: photo.uri, type: "image/jpeg", name: "photo.jpg" });
      ...
      const res = await fetch("https://api.openai.com/v1/images/edits", {
        method: "POST",
        headers: {
          // No Content-Type here — fetch sets the multipart boundary automatically for FormData.
      ...
      console.log("Response keys:", Object.keys(data).join(", "));
      if (data.error) console.log("API Error:", JSON.stringify(data.error));
      if (data.error) { Alert.alert("Visualization failed", data.error.message + " (code: " + data.error.code + ")"); return; }
      // gpt-image-2 returns base64, dall-e-3 returns url
      const b64 = data.data?.[0]?.b64_json;
      const url = data.data?.[0]?.url;
      const rawImage = b64 ? ("data:image/png;base64," + b64) : url;
  ...
  const openProduct = (q) => Linking.openURL(`https://www.google.com/search?q=${encodeURIComponent(q)}&tbm=shop`);
  ...
  const reset = () => { setPhoto(null); setResults(null); setErr(null); setBudget(""); setTierTouched(false); setVizImage({}); setVizLoading({}); setPhotoSize({width:1,height:1}); setVizModal(null); setVizModal(null); setCurrentPlanId(null); };
  const goHome = () => { setShowMenu(false); setShowHistory(false); setShowFaq(false); setShowAccount(false); setResults(null); setPhoto(null); setErr(null); setVizImage({}); setVizLoading({}); setCurrentPlanId(null); };
  ...
  const handleSignOut = () => {
    ...
    Alert.alert("Sign Out", "Are you sure you want to sign out?", [
      ...
      ...(__DEV__ ? [{ text: "🔧 Reset Test Data", onPress: async () => {
    ...
    return (
      <SafeAreaView style={s.safe}>
        ...
        <ScrollView contentContainerStyle={[s.scrollContent, {alignItems:"center"}]}>
          ...
          <Text style={s.paywallCtaSub}>Cancel anytime • Managed by Apple</Text>
            ...
            history.map((item) => (
              <TouchableOpacity key={item.id} style={s.historyItem} onPress={() => {
                Alert.alert(item.spaceType, "What would you like to do?", [
                  { text: "View Full Plan", onPress: () => { console.log("Opening plan", item.id, "vizImages:", JSON.stringify(item.vizImages)); setResults(item); setVizImage(item.vizImages || {}); setVizLoading({}); setCurrentPlanId(item.id); setShowHistory(false); } },
                  { text: "Share as PDF", onPress: () => { setResults(item); setVizImage(item.vizImages || {}); setVizLoading({}); setCurrentPlanId(item.id); setTimeout(() => generatePDF(), 100); } },
                  { text: "Cancel", style: "cancel" },
                ]);
              }}>
    ...
    return (
      <SafeAreaView style={s.safe}>
        ...
        <ScrollView contentContainerStyle={s.scrollContent}>
          ...
          <View style={s.accountInfoCard}>
            <TouchableOpacity style={[s.accountRow, {borderBottomWidth:0}]} onPress={handleSignOut}>
          ...
          {results.tiers?.map(t => {
            ...
            return (
              <View key={t.id} style={[s.tcard, {borderColor: isSelectedTier ? m.color : m.border}, isSelectedTier && {borderWidth:2.5}]}>
                ...
                {t.products?.map((p,i) => (
                  <TouchableOpacity key={i} style={s.prodRow} onPress={() => openProduct(p.searchQuery)}>
        ...
        <View style={s.tiersRow}>
          {TIERS.map(t => (
            <TouchableOpacity key={t.id}
              ...
              onPress={() => { setTier(t.id); setTierTouched(true); }}>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
      Purchases.configure({ apiKey: "appl_SIucLbhCtkbSMSuMrhGyxsfWmxx" });
