...
**Known gap (found Jul 2026, during My Plans deletion work):** account deletion's Storage cleanup only walks the `viz/{uid}/**` prefix (`listAll` + delete on `viz/{uid}`, recursing one level into per-plan subfolders). It never touches `plans/{uid}/**` — original photos and progress photos uploaded during Companion batches. Every account deleted today leaves that prefix's objects orphaned in Storage indefinitely. Not fixed as part of the My Plans single-plan-delete feature (which correctly cleans both prefixes for the plan being deleted) since fixing account deletion's broader gap is out of that feature's scope. Fix: add `plans/{uid}` to the same `listAll`+delete pass `handleConfirmDelete` already does for `viz/{uid}`.
